"use strict";
var ts_api_guardian_1 = require('ts-api-guardian');
// =================================================================================================
// =================================================================================================
...
var CORE = [
    ...
    'QueryList.dirty:any',
    'QueryList.filter(fn:(item: T, index: number, array: T[]) => boolean):T[]',
    'QueryList.first:T',
    'QueryList.forEach(fn:(item: T, index: number, array: T[]) => void):void',
    'QueryList.last:T',
    'QueryList.length:number',
    'QueryList.map(fn:(item: T, index: number, array: T[]) => U):U[]',
    'QueryList.notifyOnChanges():void',
    'QueryList.reduce(fn:(prevValue: U, curValue: T, curIndex: number, array: T[]) => U, init:U):U',
...
var PLATFORM_BROWSER_DYNAMIC_TESTING = [
...
function check(file, expected) {
    it('should check ' + file, function () { return checkPublicApi(file, expected); });
}
function checkPublicApi(file, expected) {
    var sortedActual = ts_api_guardian_1.publicApi('modules/' + file).sort();
    var sortedExpected = expected.slice().sort(function (a, b) { return a.localeCompare(b); });
    var unsorted = expected.find(function (element, index) { return element !== sortedExpected[index]; });
    var missing = sortedActual.filter(function (i) { return sortedExpected.indexOf(i) < 0; });
    var extra = sortedExpected.filter(function (i) { return sortedActual.indexOf(i) < 0; });
    ...
    if (missing.length > 0) {
        ...
        console.log('=================================================================');
        console.log('Missing:');
        missing.forEach(function (m) { return console.log(m); });
    }
    if (extra.length > 0) {
        console.log('Extra:');
        extra.forEach(function (m) { return console.log(m); });
    }
    expect(missing.map(function (s) { return ("+" + s); }).concat(extra.map(function (s) { return ("-" + s); }))).toEqual([]);
}
