...
var broccoli_check_imports_1 = require('../broccoli-check-imports');
var kServedPaths = [
...
module.exports = function makeBrowserTree(options, destinationPath) {
    ...
    var noTypeChecks = options.noTypeChecks;
    var generateEs6 = options.generateEs6;
    var sourceMaps = options.sourceMaps;
    var useBundles = options.useBundles;
    if (modules.angular2) {
    ...
    if (modules.benchmarks) {
        var benchmarksTree = new Funnel('modules/benchmarks', { include: ['**/**'], exclude: ['e2e_test/**'], destDir: '/benchmarks/' });
    }
    if (modules.benchmarks_external) {
        var benchmarksExternalTree = new Funnel('modules/benchmarks_external', { include: ['**/**'], exclude: ['e2e_test/**'], destDir: '/benchmarks_external/' });
    }
    if (modules.payload_tests) {
        var payloadTestsTree = new Funnel('modules/payload_tests', { include: ['**/ts/**'], exclude: ['e2e_test/**'], destDir: '/payload_tests/' });
    }
    if (modules.playground) {
        var playgroundTree = new Funnel('modules/playground', { include: ['**/**'], exclude: ['e2e_test/**'], destDir: '/playground/' });
    }
    if (modules.benchpress) {
        var benchpressTree = new Funnel('modules/benchpress', { include: ['**/**'], exclude: ['e2e_test/**'], destDir: '/benchpress/' });
    }
    var externalTypings = new Funnel('node_modules', { include: ['rxjs/**/*.d.ts', 'zone.js/**/*.d.ts'] });
    var modulesTree = broccoli_merge_trees_1.default([
        angular2Tree,
        benchmarksTree,
    ...
    var es6PolyfillTypings = new Funnel('modules', { include: ['angular2/typings/es6-*/**'], destDir: '/' });
    var es5ModulesTree = broccoli_merge_trees_1.default([modulesTree, es6PolyfillTypings]);
    ...
    modulesTree = broccoli_check_imports_1.default(modulesTree);
    ...
    var servingTrees = kServedPaths.reduce(getServedFunnels, []);
    ...
    if (modules.benchmarks || modules.benchmarks_external || modules.playground) {
        var assetsTree = new Funnel(modulesTree, { include: ['**/*'], exclude: ['**/*.{html,ts,dart}'], destDir: '/' });
    }
    ...
    if (modules.playground) {
        htmlTree = broccoli_replace_1.default(htmlTree, {
    ...
    if (modules.benchmarks || modules.benchmarks_external) {
        var scripts = broccoli_merge_trees_1.default(servingTrees);
    }
    if (modules.benchmarks_external) {
        ...
        var polymer = stew.mv(broccoli_flatten_1.default(polymerFiles), 'benchmarks_external/src/tree/polymer');
        var reactFiles = new Funnel('.', { files: ['node_modules/react/dist/react.min.js'] });
        var react = stew.mv(broccoli_flatten_1.default(reactFiles), 'benchmarks_external/src/tree/react');
    }
    if (modules.benchmarks || modules.benchmarks_external || modules.playground) {
        htmlTree = broccoli_merge_trees_1.default([htmlTree, scripts, polymer, react]);
    }
    // this is needed only for creating a bundle
    // typescript resolves dependencies automatically
    if (modules.bundle_deps) {
        var nodeModules = new Funnel('node_modules', { include: ['rxjs/**/**', 'parse5/**/**', 'css/**/**'], destDir: '/' });
    }
    if (generateEs6) {
        ...
        es6Tree = stew.mv(broccoli_merge_trees_1.default([es6Tree, htmlTree, assetsTree, nodeModules]), '/es6');
    }
    es5Tree = stew.mv(broccoli_merge_trees_1.default([es5Tree, htmlTree, assetsTree, nodeModules]), '/es5');
    var mergedTree = broccoli_merge_trees_1.default([es6Tree, es5Tree]);
