...
export class RuntimeCompiler {
    ...
    _compileNgModule(moduleType) {
        ...
        if (!ngModuleFactory) {
            ...
            let boundCompilerFactory = (parentResolver) => new CoreCompiler(this, moduleMeta.type.runtime, parentResolver, this._console);
            ...
            compileResult.dependencies.forEach((dep) => {
                dep.placeholder.runtime =
                    this._assertComponentLoaded(dep.comp.runtime, true).proxyComponentFactory;
    ...
    _compileComponentInModule(compType, isSync, moduleType) {
        ...
        const componentPromise = this._compileComponents(moduleType, isSync);
        const componentFactory = this._assertComponentLoaded(compType, true).proxyComponentFactory;
        ...
        ngModule.transitiveModule.modules.forEach((localModuleMeta) => {
            localModuleMeta.declaredDirectives.forEach((dirMeta) => {
                if (dirMeta.isComponent) {
                    const template = this._createCompiledTemplate(dirMeta, localModuleMeta.transitiveModule.directives, localModuleMeta.transitiveModule.pipes);
                    if (!templates.has(template)) {
                        templates.add(template);
                    }
                    dirMeta.precompile.forEach((precompileType) => {
                        const template = this._createCompiledHostTemplate(precompileType.runtime);
                        if (!templates.has(template)) {
                            templates.add(template);
                        }
                    });
